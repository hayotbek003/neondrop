...
class Command(BaseCommand):
    help = 'Populates the database with initial CS:GO cases, skins, demo user Smoke, inventory, and openings.'

    def handle(self, *args, **options):
        self.stdout.write(self.style.NOTICE("Initializing NEONDROP demo dataset..."))
        ...
        self.stdout.write(f"[OK] Created {len(items_map)} weapon skins.")
        ...
        self.stdout.write(f"[OK] Created {len(cases_data)} cases with contents and drop weights.")

        # 4. Demo User "Smoke" from reference screenshot
        ...
        # Seed initial inventory for Smoke (as shown in panel 9 of reference)
        inventory_samples = [
            'AK-47 | Neon Rider',
        ...
        # Clear existing unsold inventory for idempotent rerun
        InventoryItem.objects.filter(user=user).delete()
        for sk_name in inventory_samples:
            if sk_name in items_map:
                InventoryItem.objects.create(
                    user=user,
                    item=items_map[sk_name],
                    source='case'
                )

        # Seed initial opening history for Smoke (as shown in panel 10 of reference)
        Opening.objects.filter(user=user).delete()
        history_samples = [
            ('cyber-case', 'AK-47 | Neon Rider', Decimal('4.99')),
            ('frost-case', 'M4A1-S | Hyper Beast', Decimal('19.99')),
            ('neon-case', 'USP-S | Cortex', Decimal('2.49')),
        ]
        for c_slug, i_name, c_price in history_samples:
            case_obj = Case.objects.get(slug=c_slug)
            item_obj = items_map[i_name]
            Opening.objects.create(
                user=user,
                case=case_obj,
                item=item_obj,
                price=c_price,
                server_seed_hash=secrets.token_hex(32),
