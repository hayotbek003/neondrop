...
class Contract(models.Model):
    ...
    total_input_value = models.DecimalField(max_digits=12, decimal_places=2, verbose_name="Сумма вложенных предметов ($)")
    ...
    def __str__(self):
        return f"Контракт #{self.id} от {self.user.username}: {self.items_count} предм. (${self.total_input_value}) -> {self.reward_item.name} (${self.reward_item.value})"
